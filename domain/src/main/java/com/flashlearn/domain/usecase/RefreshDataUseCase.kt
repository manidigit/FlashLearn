...
class RefreshDataUseCase @Inject constructor(private val versions:DataVersionRepository,private val conceptRepository:ConceptRepository,private val contentRepository:ContentRepository,private val database:FlashLearnDatabase){
 ...
 suspend operator fun invoke():DataMigrationResult=database.withTransaction{
  var cv=versions.getConceptDataVersion();var tv=versions.getContentDataVersion();val cf=cv;val tf=tv;var changed=0
  while(cv<CURRENT_CONCEPT_DATA_VERSION){cv=migrateConcept(cv);versions.setConceptDataVersion(cv)}
  while(tv<CURRENT_CONTENT_DATA_VERSION){val r=migrateContent(tv);tv=r.first;changed+=r.second;versions.setContentDataVersion(tv)}
  require(cv==CURRENT_CONCEPT_DATA_VERSION){"Unsupported concept data version: $cv"};require(tv==CURRENT_CONTENT_DATA_VERSION){"Unsupported content data version: $tv"}
