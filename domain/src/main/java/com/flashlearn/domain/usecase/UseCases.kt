...
data class SubmitReviewAnswerResult(
    val learningState: LearningState, val difficultyState: DifficultyState, val transition: TransitionResult
)

fun computeCanonicalKey(text: String): String = text.trim().lowercase().replace(Regex("\\s+"), " ")
...
class CreateConceptUseCase @Inject constructor(
    ...
    suspend operator fun invoke(command: CreateConceptCommand): UUID = database.withTransaction {
        ...
        learningStateRepository.upsert(LearningState(UUID.randomUUID(), id, Stage.DAILY, null, 0, false, 0, 0, null))
