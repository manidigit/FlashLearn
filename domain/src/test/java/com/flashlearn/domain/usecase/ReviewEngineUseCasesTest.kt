...
class ReviewEngineUseCasesTest {
    ...
    private fun concept(id: UUID, categoryId: UUID? = null, active: Boolean = true) =
        Concept(id, EntryType.WORD, categoryId, false, active, now, now)
    ...
    @Test
    fun scheduledModes_sortByOldestDueThenConceptId() = runBlocking {
        val oldest = UUID.randomUUID()
        val newer = UUID.randomUUID()
        val future = UUID.randomUUID()
        val repo = SelectReviewQueueUseCase(
            CRepo(listOf(concept(newer), concept(oldest), concept(future))),
            LRepo(listOf(
                learning(newer, Stage.WEEKLY, now.minusSeconds(60)),
                learning(oldest, Stage.WEEKLY, now.minusSeconds(120)),
                learning(future, Stage.WEEKLY, now.plusSeconds(60))
            )),
            DRepo(listOf(
                difficulty(newer, VocabularyDifficulty.EASY),
                difficulty(oldest, VocabularyDifficulty.EASY),
                difficulty(future, VocabularyDifficulty.EASY)
            )),
            TRepo(emptyMap())
        )
        ...
        assertEquals(listOf(oldest, newer), result.map { it.concept.id })
    }
    ...
    @Test
    fun dailyMode_excludesCardsWithoutScheduledReviewTime() = runBlocking {
        val scheduled = UUID.randomUUID()
        val unscheduled = UUID.randomUUID()
        val repo = SelectReviewQueueUseCase(
            CRepo(listOf(concept(scheduled), concept(unscheduled))),
            LRepo(listOf(learning(scheduled, Stage.DAILY), learning(unscheduled, Stage.DAILY, null))),
            DRepo(listOf(difficulty(scheduled, VocabularyDifficulty.EASY), difficulty(unscheduled, VocabularyDifficulty.EASY))),
            TRepo(emptyMap())
        )

        val result = repo(ReviewSelectionFilters(ReviewType.DAILY, now = now))
        assertEquals(listOf(scheduled), result.map { it.concept.id })
    }

    @Test
    fun ordinaryModes_excludeInactiveConcepts() = runBlocking {
        val activeId = UUID.randomUUID()
        val deletedId = UUID.randomUUID()
        val repo = SelectReviewQueueUseCase(
            CRepo(listOf(concept(activeId), concept(deletedId, active = false))),
            LRepo(listOf(learning(activeId, Stage.DAILY), learning(deletedId, Stage.DAILY))),
            DRepo(listOf(difficulty(activeId, VocabularyDifficulty.EASY), difficulty(deletedId, VocabularyDifficulty.EASY))),
            TRepo(emptyMap())
        )

        val result = repo(ReviewSelectionFilters(ReviewType.DAILY, now = now))

        assertEquals(listOf(activeId), result.map { it.concept.id })
    }

    @Test
    fun randomMode_neverIncludesLearnedConcepts() = runBlocking {
        val dailyId = UUID.randomUUID()
        val weeklyId = UUID.randomUUID()
        val monthlyId = UUID.randomUUID()
        val learnedId = UUID.randomUUID()
        val repo = SelectReviewQueueUseCase(
            CRepo(listOf(concept(dailyId), concept(weeklyId), concept(monthlyId), concept(learnedId))),
            LRepo(listOf(
                learning(dailyId, Stage.DAILY),
                learning(weeklyId, Stage.WEEKLY),
                learning(monthlyId, Stage.MONTHLY),
                learning(learnedId, Stage.LEARNED, null)
            )),
            DRepo(listOf(
                difficulty(dailyId, VocabularyDifficulty.EASY),
                difficulty(weeklyId, VocabularyDifficulty.MEDIUM),
                difficulty(monthlyId, VocabularyDifficulty.HARD),
                difficulty(learnedId, VocabularyDifficulty.EASY)
            )),
            TRepo(emptyMap())
        )
        ...
        assertEquals(setOf(dailyId, weeklyId, monthlyId), result.map { it.concept.id }.toSet())
        assertEquals(3, result.size)
    }

    @Test
    fun learnedMode_includesOnlyLearnedAndDoesNotApplyDueDateGate() = runBlocking {
        val learnedId = UUID.randomUUID()
        val futureLearnedId = UUID.randomUUID()
        val dailyId = UUID.randomUUID()
        val repo = SelectReviewQueueUseCase(
            CRepo(listOf(concept(learnedId), concept(futureLearnedId), concept(dailyId))),
            LRepo(listOf(
                learning(learnedId, Stage.LEARNED, now.plusSeconds(3600)),
                learning(futureLearnedId, Stage.LEARNED, now.plusSeconds(7200)),
                learning(dailyId, Stage.DAILY)
            )),
            DRepo(listOf(
                difficulty(learnedId, VocabularyDifficulty.EASY),
                difficulty(futureLearnedId, VocabularyDifficulty.EASY),
                difficulty(dailyId, VocabularyDifficulty.EASY)
            )),
            TRepo(emptyMap())
        )
        ...
        assertEquals(setOf(learnedId, futureLearnedId), result.map { it.concept.id }.toSet())
