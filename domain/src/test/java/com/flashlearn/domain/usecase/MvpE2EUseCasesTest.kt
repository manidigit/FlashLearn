...
class MvpE2EUseCasesTest {
    ...
    fun addWordThenReviewThenProgress_isConsistent() = runBlocking {
        ...
        val home = GetProgressSummaryUseCase(concepts, learning)(now)
        ...
        val result = submit(SubmitReviewAnswerRequest(ids.first(), sessionId, UUID.randomUUID(), ReviewType.DAILY, true, now))
        ...
        val refreshedHome = GetProgressSummaryUseCase(concepts, learning)(now)
        ...
        val afterDelete = GetProgressSummaryUseCase(concepts, learning)(now)
        ...
        val queue = SelectReviewQueueUseCase(concepts, learning, difficulty, Tags())(
            ReviewSelectionFilters(ReviewType.RANDOM, now = now)
        )
