...
class BackupValidatorTest {
    ...
    @Test fun rejectsParserMetadataForMissingConcept() {
        val metadata = BackupParserMetadata(UUID.randomUUID(), "{}", "[]", "[]", 0.8)
        val data = BackupData(1, Instant.now(), BackupType.FULL, concepts=listOf(concept), parserMetadata=listOf(metadata))
        assertFalse(BackupValidator.validate(data, 1).valid)
    }

    @Test fun rejectsInvalidParserMetadataConfidence() {
        val metadata = BackupParserMetadata(concept.id, "{}", "[]", "[]", 1.1)
        val data = BackupData(1, Instant.now(), BackupType.FULL, concepts=listOf(concept), parserMetadata=listOf(metadata))
        assertFalse(BackupValidator.validate(data, 1).valid)
    }

    @Test fun acceptsValidParserMetadata() {
        val metadata = BackupParserMetadata(concept.id, "{}", "[]", "[]", 0.8)
        val data = BackupData(1, Instant.now(), BackupType.FULL, concepts=listOf(concept), parserMetadata=listOf(metadata))
        assertTrue(BackupValidator.validate(data, 1).valid)
    }
}
