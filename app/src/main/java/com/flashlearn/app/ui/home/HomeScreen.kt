        ...
        Card(Modifier.fillMaxWidth(), shape = MaterialTheme.shapes.large) {
            Row(Modifier.fillMaxWidth().padding(18.dp), verticalAlignment = Alignment.CenterVertically) {
                ...
                Column(Modifier.weight(1f)) {
                    ...
                    Text("کلمه منتظر شماست", color = MaterialTheme.colorScheme.onSurfaceVariant)
                }
            }
        }
        ...
        Card(Modifier.fillMaxWidth(), shape = MaterialTheme.shapes.large) {
            Column(Modifier.padding(16.dp), verticalArrangement = Arrangement.spacedBy(10.dp)) {
                Row(verticalAlignment = Alignment.CenterVertically) {
                    ...
                    Text("روزانه", style = MaterialTheme.typography.titleMedium, modifier = Modifier.weight(1f))
                    Text("$daily از $due", color = MaterialTheme.colorScheme.onSurfaceVariant)
                }
...
private fun QuickAction(label: String, icon: androidx.compose.ui.graphics.vector.ImageVector, modifier: Modifier, onClick: () -> Unit) {
    Card(modifier, onClick = onClick, shape = MaterialTheme.shapes.medium) {
