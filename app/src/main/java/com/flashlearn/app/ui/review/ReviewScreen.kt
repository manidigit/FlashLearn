...
@Composable private fun QuizCard(state: ReviewUiState, vm: ReviewViewModel) {
    ...
    if (!answered) {
        ...
        Surface(modifier = Modifier.fillMaxWidth(), shape = MaterialTheme.shapes.large, color = if (feedback?.isCorrect == true) QuizCorrectContainer else QuizWrongContainer) {
            Text(if (feedback?.isCorrect == true) "✓ پاسخ صحیح — چند لحظه صبر کنید…" else "✕ پاسخ غلط — پاسخ صحیح سبز شده است؛ چند لحظه صبر کنید…", color = if (feedback?.isCorrect == true) QuizCorrect else QuizWrong, style = MaterialTheme.typography.titleMedium, fontWeight = FontWeight.Bold, textAlign = TextAlign.Center, modifier = Modifier.fillMaxWidth().padding(horizontal = 14.dp, vertical = 12.dp))
        }
    }
}
