...
private fun ReviewSetup(state: ReviewUiState, vm: ReviewViewModel, personalDifficulty: VocabularyDifficulty?, onBack: () -> Unit = {}, onOpenCategories: () -> Unit) {
    Column(modifier = Modifier.fillMaxWidth(), verticalArrangement = Arrangement.spacedBy(18.dp)) {
        Box(Modifier.fillMaxWidth().height(58.dp)) {
            IconButton(onClick = onBack, modifier = Modifier.align(Alignment.CenterStart)) { Icon(mdiIcon("arrow-left"), contentDescription = "بازگشت", tint = MaterialTheme.colorScheme.onSurface) }
        ...
        Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(10.dp)) {
            ReviewModeCard("تستی", state.selectedMode == ReviewMode.QUIZ, { vm.chooseMode(ReviewMode.QUIZ) }, mdiIcon("help-circle-outline"), Modifier.weight(1f))
            ReviewModeCard("فلش‌کارت", state.selectedMode == ReviewMode.FLASHCARD, { vm.chooseMode(ReviewMode.FLASHCARD) }, mdiIcon("shape-outline"), Modifier.weight(1f))
        }
        ...
        Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(10.dp)) {
            ReviewModeCard("تصادفی", state.selectedReviewType == ReviewType.RANDOM, { vm.chooseReviewType(ReviewType.RANDOM) }, mdiIcon("shape-outline"), Modifier.weight(1f))
            ReviewModeCard("یادگرفته", state.selectedReviewType == ReviewType.LEARNED, { vm.chooseReviewType(ReviewType.LEARNED) }, mdiIcon("check-all"), Modifier.weight(1f))
        }
        ...
        Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(10.dp)) {
            ReviewModeCard("روزانه", state.selectedReviewType == ReviewType.DAILY, { vm.chooseReviewType(ReviewType.DAILY) }, mdiIcon("format-list-numbered"), Modifier.weight(1f))
            ReviewModeCard("هفتگی", state.selectedReviewType == ReviewType.WEEKLY, { vm.chooseReviewType(ReviewType.WEEKLY) }, mdiIcon("shape-outline"), Modifier.weight(1f))
            ReviewModeCard("ماهانه", state.selectedReviewType == ReviewType.MONTHLY, { vm.chooseReviewType(ReviewType.MONTHLY) }, mdiIcon("creation"), Modifier.weight(1f))
        }
        ...
        if (state.selectedMode == ReviewMode.QUIZ) {
            ...
            Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(10.dp)) {
                QuizLevelTile("ابتدایی", state.selectedQuizDifficulty == QuizDifficulty.EASY, mdiIcon("school-outline"), Modifier.weight(1f)) { vm.chooseQuizDifficulty(QuizDifficulty.EASY) }
                QuizLevelTile("متوسط", state.selectedQuizDifficulty == QuizDifficulty.MEDIUM, mdiIcon("tune"), Modifier.weight(1f)) { vm.chooseQuizDifficulty(QuizDifficulty.MEDIUM) }
                QuizLevelTile("حرفه‌ای", state.selectedQuizDifficulty == QuizDifficulty.HARD, mdiIcon("trophy-outline"), Modifier.weight(1f)) { vm.chooseQuizDifficulty(QuizDifficulty.HARD) }
            }
        }
...
@Composable private fun DifficultyTile(label: String, selected: Boolean, difficulty: VocabularyDifficulty, icon: com.mikepenz.iconics.typeface.IIcon, modifier: Modifier, vm: ReviewViewModel) {
    ...
    Surface(modifier.height(128.dp).clickable { vm.toggleDifficulty(difficulty) }, shape = MaterialTheme.shapes.extraLarge, color = if (selected) tint.copy(alpha = .07f) else MaterialTheme.colorScheme.surface, border = BorderStroke(if (selected) 2.dp else 1.dp, if (selected) tint.copy(alpha = .55f) else MaterialTheme.colorScheme.outlineVariant.copy(alpha = .7f))) { Box(Modifier.fillMaxSize()) { if (selected) Icon(mdiIcon("check-all"), null, tint = tint, modifier = Modifier.align(Alignment.TopEnd).padding(7.dp).size(18.dp)); Column(Modifier.fillMaxSize().padding(8.dp), horizontalAlignment = Alignment.CenterHorizontally, verticalArrangement = Arrangement.Center) { Icon(icon, null, tint = tint, modifier = Modifier.size(32.dp)); Spacer(Modifier.height(5.dp)); Text(label, style = MaterialTheme.typography.labelLarge, fontWeight = if (selected) FontWeight.Bold else FontWeight.Normal) } } }
}

@Composable
private fun difficultyIcon(difficulty: VocabularyDifficulty) = when (difficulty) { VocabularyDifficulty.EASY -> mdiIcon("star"); VocabularyDifficulty.MEDIUM -> mdiIcon("creation"); VocabularyDifficulty.HARD -> mdiIcon("fire-station"); VocabularyDifficulty.VERY_HARD -> mdiIcon("rocket-launch-outline") }

@Composable private fun QuizLevelTile(label: String, selected: Boolean, icon: com.mikepenz.iconics.typeface.IIcon, modifier: Modifier, onClick: () -> Unit) { Surface(modifier.height(96.dp).clickable(onClick = onClick), shape = MaterialTheme.shapes.large, color = if (selected) QuizSelectedContainer else MaterialTheme.colorScheme.surface, border = BorderStroke(if (selected) 2.dp else 1.dp, if (selected) QuizSelected.copy(alpha = .7f) else MaterialTheme.colorScheme.outlineVariant)) { Box(Modifier.fillMaxSize()) { if (selected) Icon(mdiIcon("check-all"), null, tint = QuizSelected, modifier = Modifier.align(Alignment.TopEnd).padding(7.dp).size(18.dp)); Column(Modifier.fillMaxSize().padding(8.dp), horizontalAlignment = Alignment.CenterHorizontally, verticalArrangement = Arrangement.Center) { Icon(icon, null, tint = if (selected) QuizSelected else MaterialTheme.colorScheme.onSurfaceVariant, modifier = Modifier.size(30.dp)); Spacer(Modifier.height(5.dp)); Text(label, style = MaterialTheme.typography.labelLarge, fontWeight = if (selected) FontWeight.Bold else FontWeight.Normal) } } } }
...
@Composable private fun QuizCard(state: ReviewUiState, vm: ReviewViewModel) {
    ...
    Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.Center, verticalAlignment = Alignment.CenterVertically) {
        Surface(shape = MaterialTheme.shapes.large, color = QuizWrongContainer.copy(alpha = .9f)) { Row(verticalAlignment = Alignment.CenterVertically, modifier = Modifier.padding(horizontal = 12.dp, vertical = 7.dp)) { Text("${state.wrong}", color = QuizWrong, style = MaterialTheme.typography.titleMedium, fontWeight = FontWeight.Bold); Spacer(Modifier.width(5.dp)); Icon(mdiIcon("close"), contentDescription = null, tint = QuizWrong) } }
        ...
        Spacer(Modifier.width(10.dp))
        Surface(shape = MaterialTheme.shapes.large, color = QuizCorrectContainer.copy(alpha = .9f)) { Row(verticalAlignment = Alignment.CenterVertically, modifier = Modifier.padding(horizontal = 12.dp, vertical = 7.dp)) { Text("${state.correct}", color = QuizCorrect, style = MaterialTheme.typography.titleMedium, fontWeight = FontWeight.Bold); Spacer(Modifier.width(5.dp)); Icon(mdiIcon("check"), contentDescription = null, tint = QuizCorrect) } }
    }
    ...
    Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.spacedBy(8.dp)) {
        ...
        OutlinedButton(onClick = vm::revealHint, enabled = !answered && card != null && !card.hintRevealed, modifier = Modifier.weight(1f)) { Row(verticalAlignment = Alignment.CenterVertically) { Icon(mdiIcon("lightbulb-outline"), contentDescription = null); Spacer(Modifier.width(6.dp)); Text("راهنما") } }
    }
    Column(Modifier.fillMaxWidth(), verticalArrangement = Arrangement.spacedBy(12.dp)) {
        quiz.options.forEach { option ->
            ...
            OutlinedButton(onClick = { vm.selectQuizOption(option) }, enabled = !answered && !state.isSubmitting, modifier = Modifier.fillMaxWidth().height(78.dp), shape = MaterialTheme.shapes.large, border = BorderStroke(if (emphasized) 3.dp else 1.dp, border), colors = ButtonDefaults.outlinedButtonColors(containerColor = container, contentColor = content), contentPadding = PaddingValues(horizontal = 18.dp, vertical = 10.dp)) {
                Row(Modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.Center) {
                    when { answered && isCorrect -> Icon(mdiIcon("check"), contentDescription = null, tint = QuizCorrect, modifier = Modifier.size(24.dp)); isWrongSelection -> Icon(mdiIcon("close"), contentDescription = null, tint = QuizWrong, modifier = Modifier.size(24.dp)); isSelected -> Icon(mdiIcon("circle"), contentDescription = null, tint = QuizSelected, modifier = Modifier.size(14.dp)) }
    ...
    if (!answered) {
        ...
        Surface(modifier = Modifier.fillMaxWidth(), shape = MaterialTheme.shapes.large, color = if (feedback?.isCorrect == true) QuizCorrectContainer else QuizWrongContainer) {
            Row(Modifier.fillMaxWidth().padding(horizontal = 14.dp, vertical = 12.dp), horizontalArrangement = Arrangement.Center, verticalAlignment = Alignment.CenterVertically) { Icon(mdiIcon(if (feedback?.isCorrect == true) "check-circle-outline" else "close-circle-outline"), contentDescription = null, tint = if (feedback?.isCorrect == true) QuizCorrect else QuizWrong); Spacer(Modifier.width(8.dp)); Text(if (feedback?.isCorrect == true) "پاسخ صحیح — چند لحظه صبر کنید…" else "پاسخ غلط — پاسخ صحیح سبز شده است؛ چند لحظه صبر کنید…", color = if (feedback?.isCorrect == true) QuizCorrect else QuizWrong, style = MaterialTheme.typography.titleMedium, fontWeight = FontWeight.Bold, textAlign = TextAlign.Center) }
        }
    }
}
...
@Composable private fun FeedbackCard(state: ReviewUiState) { val f = state.answerFeedback ?: return; Card(Modifier.fillMaxWidth(), shape = MaterialTheme.shapes.large) { Column(Modifier.padding(22.dp), horizontalAlignment = Alignment.CenterHorizontally, verticalArrangement = Arrangement.spacedBy(7.dp)) { Row(verticalAlignment = Alignment.CenterVertically) { Icon(mdiIcon(if (f.isCorrect) "check-circle-outline" else "close-circle-outline"), contentDescription = null, tint = if (f.isCorrect) QuizCorrect else QuizWrong); Spacer(Modifier.width(8.dp)); Text(if (f.isCorrect) "پاسخ صحیح" else "پاسخ نادرست", style = MaterialTheme.typography.headlineSmall, color = if (f.isCorrect) QuizCorrect else QuizWrong) }; Text("مرحله: ${f.stageLabel}"); Text("سختی: ${f.difficultyLabel}"); if (!f.isCorrect && !f.correctAnswerText.isNullOrBlank()) Text("پاسخ صحیح: ${f.correctAnswerText}"); Text("نتیجه: ${state.correct} صحیح، ${state.wrong} غلط") } } }
