...
class BackupViewModel @Inject constructor(
    ...
    fun export(type: BackupType = BackupType.FULL) = viewModelScope.launch {
        ...
        runCatching {
            ...
            _state.value = BackupUiState(
                message = "ساخت پشتیبان ناموفق بود: ${error.message ?: "خطای نامشخص"}"
            )
        }
    }
    ...
    fun exportData(format: ExportFormat) = viewModelScope.launch {
        ...
        runCatching {
            ...
            _state.value = _state.value.copy(
                busy = false,
                message = "ساخت خروجی ناموفق بود: ${error.message ?: "خطای نامشخص"}"
            )
        }
    }
    ...
    fun restore(json: String, onSuccess: () -> Unit = {}) = viewModelScope.launch {
        ...
        runCatching {
            withContext(Dispatchers.IO) {
                ...
                when {
                    root.optString("backupMode") == "VOCABULARY" -> {
                        vocabularyRepo.restore(json)
                    }
            ...
            _state.value = BackupUiState(
                message = "بازیابی ناموفق بود: ${error.message ?: "خطای نامشخص"}"
            )
        }
    }
