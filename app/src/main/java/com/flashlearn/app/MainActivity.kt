...
class MainActivity : ComponentActivity() {
    ...
    @Composable private fun AppRootScreen(){ val uiState by appViewModel.state; val topLevel=uiState.selectedRoute in setOf(AppRoutes.HOME,AppRoutes.REVIEW,AppRoutes.LIBRARY,AppRoutes.PROGRESS,AppRoutes.SETTINGS); if(topLevel){ FlashLearnShell(selectedRoute=uiState.selectedRoute,onNavigate={route->appViewModel.navigate(route);when(route){AppRoutes.HOME->homeViewModel.refresh();AppRoutes.LIBRARY->{libraryViewModel.setLanguagePair(uiState.languagePair);libraryViewModel.refresh()};AppRoutes.PROGRESS->progressViewModel.refresh();else->Unit}}){TopLevelContent(uiState.selectedRoute)} } else when(uiState.selectedRoute){
        ...
        AppRoutes.ADD_WORD->AddWordMethodScreen(onBack={appViewModel.navigate(AppRoutes.LIBRARY)},onSingleWord={appViewModel.navigate(AppRoutes.ADD_WORD_FORM)},onBulkWords={appViewModel.navigate(AppRoutes.BULK_IMPORT)},onRestore={appViewModel.navigate(AppRoutes.BACKUP)})
        AppRoutes.ADD_WORD_FORM->AddWordScreen(addWordViewModel,languagePair=uiState.languagePair,onBack={appViewModel.navigate(AppRoutes.ADD_WORD)},onBulkImport={appViewModel.navigate(AppRoutes.BULK_IMPORT)},onBackupRestore={appViewModel.navigate(AppRoutes.BACKUP)})
        ...
        AppRoutes.BACKUP->BackupScreen(backupViewModel,onBack={appViewModel.navigate(AppRoutes.ADD_WORD)},onRestored={homeViewModel.refresh();libraryViewModel.refresh();progressViewModel.refresh()})
        AppRoutes.CATEGORY_SELECTION->CategorySelectionScreen(categories=libraryViewModel.state.value.categories,selectedIds=libraryViewModel.state.value.selectedCategoryId?.let { setOf(it) } ?: emptySet(),counts=libraryViewModel.state.value.categoryCounts,onBack={appViewModel.navigate(AppRoutes.LIBRARY)},onApply={ids->libraryViewModel.onCategoryChange(ids.singleOrNull());appViewModel.navigate(AppRoutes.LIBRARY)})
    } }
