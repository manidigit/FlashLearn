...
class MainActivity : ComponentActivity() {
    ...
    private fun AppRootScreen() {
        ...
        if (topLevel) {
            ...
            when (uiState.selectedRoute) {
                ...
                AppRoutes.ADD_WORD -> FlashLearnShell(
    ...
    private fun TopLevelContent(route: String) {
        ...
        when (route) {
            ...
            AppRoutes.REVIEW -> ReviewSessionContent(
                viewModel = reviewViewModel,
                personalDifficulty = uiState.personalWordDifficulty,
    ...
    private fun ReviewSessionContent(
        viewModel: ReviewViewModel,
        personalDifficulty: com.flashlearn.domain.model.VocabularyDifficulty?,
        quizDifficulty: com.flashlearn.domain.model.QuizDifficulty,
        onFinished: () -> Unit
    ) {
        val reviewState by viewModel.state.collectAsState()
        val sessionActive = !reviewState.isSelectingMode && !reviewState.isFinished
        val exitToSetup = {
            viewModel.exitReview { viewModel.prepareReviewType(reviewState.selectedReviewType) }
        }
        if (sessionActive) {
            BackHandler(onBack = exitToSetup)
        }
        Column(Modifier.fillMaxSize()) {
            if (sessionActive) {
                Surface(
                    ...
                    Row(Modifier.fillMaxWidth().padding(horizontal = 6.dp, vertical = 2.dp), verticalAlignment = Alignment.CenterVertically) {
                        IconButton(onClick = exitToSetup) {
                            Icon(Icons.Outlined.Close, contentDescription = "بستن مرور")
                        }
                        Spacer(Modifier.weight(1f))
                        Column(horizontalAlignment = Alignment.End) {
                            Text("مرور کلمات", style = MaterialTheme.typography.titleMedium, fontWeight = androidx.compose.ui.text.font.FontWeight.Bold)
            ...
            Box(Modifier.weight(1f).fillMaxWidth()) {
                ReviewScreen(viewModel, personalDifficulty = personalDifficulty, quizDifficulty = quizDifficulty, onFinished = onFinished)
            }
        }
    }
}
