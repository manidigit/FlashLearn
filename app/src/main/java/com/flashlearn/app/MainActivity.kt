...
class MainActivity:ComponentActivity(){
 ...
 override fun onCreate(savedInstanceState:Bundle?){super.onCreate(savedInstanceState);setContent{val appState by appViewModel.state;FlashLearnTheme(appearance=appState.appearance,themeId=appState.themeId){CompositionLocalProvider(LocalLayoutDirection provides if(appState.layoutDirection==AppLayoutDirection.RTL)LayoutDirection.Rtl else LayoutDirection.Ltr){Surface(Modifier.fillMaxSize()){BackHandler{if(appViewModel.state.value.selectedRoute==AppRoutes.HOME)finish()else appViewModel.goBack()};AppRootScreen()}}}}}
