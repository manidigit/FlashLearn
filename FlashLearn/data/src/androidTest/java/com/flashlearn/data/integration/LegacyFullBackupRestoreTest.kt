...
class LegacyFullBackupRestoreTest {
    ...
    @Test fun restoresLegacyFullShapeIncludingProgressHistoryAndSettings() = runBlocking {
        ...
        val concept = JSONObject()
            .put("uuid", conceptId.toString()).put("contentType", "WORD")
            .put("categoryName", "اعضای بدن").put("favorite", true).put("active", true)
            .put("createdAt", exported - 10_000L).put("updatedAt", exported - 5_000L)
            .put("notes", "legacy note").put("tags", JSONArray()).put("contents", contents)
        val json = JSONObject()
            .put("schemaVersion", 1).put("exportedAt", exported)
            ...
            .put("concepts", JSONArray().put(concept))
            .put("learningStates", JSONArray().put(JSONObject()
                .put("conceptUuid", conceptId.toString()).put("stage", "WEEKLY").put("difficulty", "HARD")
                .put("monthlyWrongCount", 2).put("totalCorrect", 7).put("totalWrong", 3)
                .put("everFailed", true).put("consecutiveCorrect", 0).put("consecutiveIncorrect", 2).put("difficultyScore", 7)))
    ...
    @Test fun legacyRestoreIsIdempotent() = runBlocking {
        ...
        val concept = JSONObject()
            .put("uuid", conceptId.toString()).put("contentType", "WORD")
            .put("favorite", false).put("active", true).put("createdAt", now).put("updatedAt", now)
            .put("contents", contents)
        val json = JSONObject()
            .put("schemaVersion", 1).put("exportedAt", now).put("languages", JSONArray())
            .put("categories", JSONArray()).put("concepts", JSONArray().put(concept))
            .put("learningStates", JSONArray().put(JSONObject()
                .put("conceptUuid", conceptId.toString()).put("stage", "DAILY").put("difficulty", "EASY")))
