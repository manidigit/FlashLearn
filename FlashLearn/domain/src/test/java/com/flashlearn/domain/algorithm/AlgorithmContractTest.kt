...
class AlgorithmContractTest {
    ...
    @Test fun learnedReviewDoesNotMutateDifficulty() {
        val original = DifficultyState(
            id = UUID.randomUUID(),
            conceptId = UUID.randomUUID(),
            current = VocabularyDifficulty.VERY_HARD,
            consecutiveCorrect = 0,
            consecutiveWrong = 1,
            hasReachedVeryHard = true
        )
        val result = calculateDifficulty(original, true, ReviewType.LEARNED, 4)
