...
class CountReviewQueueUseCase @Inject constructor(private val conceptRepository:ConceptRepository,private val learningStateRepository:LearningStateRepository,private val difficultyStateRepository:DifficultyStateRepository,private val conceptTagRepository:ConceptTagRepository,private val reviewHistoryRepository:ReviewHistoryRepository){
    suspend operator fun invoke(filters:ReviewSelectionFilters):Int{
        ...
        return states.asSequence().mapNotNull{learning->if(learning.conceptId in practicedToday)return@mapNotNull null;val concept=conceptsById[learning.conceptId]?:return@mapNotNull null;val difficulty=difficultiesById[learning.conceptId]?:return@mapNotNull null;val tags=tagsByConcept[learning.conceptId].orEmpty();if(filters.difficulty!=null&&difficulty.current!=filters.difficulty)return@mapNotNull null;if(filters.categoryId!=null&&concept.categoryId!=filters.categoryId)return@mapNotNull null;if(filters.tagId!=null&&filters.tagId !in tags)return@mapNotNull null;concept.id}.distinct().count()
    }
}
