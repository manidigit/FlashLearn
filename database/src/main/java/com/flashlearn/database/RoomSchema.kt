...
@Dao interface ReviewHistoryDao {
    ...
    @Update suspend fun update(entity: ReviewHistoryEntity)
...
@Dao interface CategoryDao {
    ...
    @Query("SELECT * FROM categories WHERE id = :id LIMIT 1") suspend fun getById(id: UUID): CategoryEntity?
...
@Dao interface AchievementDao {
    ...
    @Insert(onConflict = OnConflictStrategy.REPLACE) suspend fun upsert(entity: AchievementEntity)
